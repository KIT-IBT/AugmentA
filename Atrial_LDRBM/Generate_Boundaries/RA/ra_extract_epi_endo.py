...
def run():
    ...
    # Read the contour("the equal potential surface")
    reader = vtk.vtkPolyDataReader()
    # reader.SetFileName('model/contour_-0.00006.vtk')
    ...
    output_points = atrial_points_tree.query_ball_tree(point_tree, 0.4)
    ...
    # Get cell ids from RA file if cell contains points on the mixed_points_list
    ...
    print('Extracting epi surface ... done\n \nWriting as ra_epi_sruface.vtk...')

    # Write epi surface as vtk file
    meshNew = dsa.WrapDataObject(epi_new)
    writer = vtk.vtkPolyDataWriter()
    writer.SetFileName("result/ra_epi_surface.vtk")
    writer.SetInputData(meshNew.VTKObject)
    writer.Write()
    print('Writing as ra_epi_surface.vtk...done\n ')
    ...
    print('Extracting endo surface ... done\n \nWriting as ra_endo_sruface.vtk...')
    meshNew = dsa.WrapDataObject(endo_extraction)
    writer = vtk.vtkUnstructuredGridWriter()
    writer.SetFileName("result/ra_endo_surface.vtk")
    writer.SetInputData(meshNew.VTKObject)
    writer.Write()
    print('Writing as ra_endo_sruface.vtk...done\n ')
